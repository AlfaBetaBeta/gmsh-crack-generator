...
def mesh2df(fname):
    ...
    df_elm = DataFrame with elements
             ...
             elm_ID    [...]   [...]   ...
               ...
               .         .       .      .
               .         .       .      .
    NOTE: The first element of tags is already the physical tag, not the total number
          of tags (as in gmsh), as this can be readily obtained via len(tags).
    ...
    # Create DataFrames
    df_phe = pd.DataFrame(phyent_dict, index=phyent_ID)
    df_nod = pd.DataFrame(node_dict, index=node_ID)
    df_elm = pd.DataFrame(elem_dict, index=elem_ID)
...
def flatten(L):
    ...
    If the input argument is already a one-level list, the generated object will
    coincide, i.e. the 'flattened' list will be equal to the original list.
    """
    for el in L:
...
def preproc_df(df_elm, crack_ids):
    ...
    crack_ids = List with crack physical tags, with some pairs being possibly coupled
                if they belong to the same crack plane (e.g. [1,[2,3],4])
                
    df_elm may be edited in here if an elementary tag is found to be repeated for
    different cracks
    Returns:
    dic_s2s = Dictionary with IDs of the same crack surface elements under different
              physical tags
    """
    L_cr = list(flatten(crack_ids)) # See previous cell for flatten() function
...
def elem_ctrd(df_nod, sr_elm, nodmap):
    ...
    nodmap  = Dictionary linking old with duplicated nodes (duplication at upper level)
              in case df_nod makes a loc call to a duplicated node before
              that DataFrame could be updated
...
def ext_nrm(df_nod, sr_elm):
    ...
    vec1 = [i-j for i,j in zip(df_nod.loc[n3[1],'coords'], df_nod.loc[n3[0],'coords'])]
    vec2 = [i-j for i,j in zip(df_nod.loc[n3[2],'coords'], df_nod.loc[n3[1],'coords'])]
    nrm = [vec1[1]*vec2[2]-vec1[2]*vec2[1],
...
def srf_split(df_nod, df_elm, elm_ID, elm_nrm, dic_s2s, nodmap):
    ...
    nodmap  = Dictionary linking old with duplicated nodes, to be passed on to
              elem_ctrd in case df_nod makes a loc call to a duplicated node before
              that DataFrame could be updated
    
    Returns:
    top_bot = list with the 2 solid element IDs (belonging to dic_s2s.keys()) attached
             (by at least 3 corner nodes) to the given surface element, always in the order
             [TOP elem_ID, BOTTOM elem_ID], where top and bottom are given by the
             orientation of the surface normal (it points bottom to top).
             Should one of the 2 elements be missing, then it is replaced by 0.
    """
    ...
    for n in range(2):
        df_elm_msk = df_elm[[n3[n+1] in set(L) for L in df_elm['nodes'].values.tolist()]]
        df_elm_msk = df_elm_msk[[i in dic_s2s.keys() for i in df_elm_msk.index]]
        
        top_bot.intersection_update(df_elm_msk.index)
    
    if len(top_bot) == 0:
        print('Error: srf_split() could not find solid elements attached to surface element {0}'.format(elm_ID))
        sys.exit(1)
    
    if len(top_bot) == 1:
        vec = [i-j for i,j in zip(elem_ctrd(df_nod,df_elm.loc[list(top_bot)[0]],nodmap),df_nod.loc[n3[0],'coords'])]
        #dp = sum([i*j for i,j in zip(vec,ext_nrm(df_nod,sr_elm))])
        dp = sum([i*j for i,j in zip(vec,elm_nrm)])
        if dp > 0: top_bot = list(top_bot) + [0]
        if dp < 0: top_bot = (list(top_bot) + [0])[::-1]
    else:
        vec = [i-j for i,j in zip(elem_ctrd(df_nod,df_elm.loc[list(top_bot)[0]],nodmap),elem_ctrd(df_nod,df_elm.loc[list(top_bot)[1]],nodmap))]
        #dp = sum([i*j for i,j in zip(vec,ext_nrm(df_nod,sr_elm))])
        dp = sum([i*j for i,j in zip(vec,elm_nrm)])
        if dp > 0: top_bot = list(top_bot)
        if dp < 0: top_bot = list(top_bot)[::-1]
    
    if type(top_bot) == set:
        print('Error: srf_split() could not convert top_bot to list')
        print('top_bot =',top_bot,'during processing of surface element {0}'.format(elm_ID))
        if dp == 0:
            print('dp = 0, which indicates that a solid element has been counted twice in top_bot')
...
def srf_neigh(df_nod, df_elm, elm_ID, elm_nrm, dic_s2s, nodmap):
    ...
    Returns:
    neigh = list with IDs of neighbouring solid elements in contact with the given
            surface element but NOT attached to it (i.e. they are NOT in the output
            of srf_split(..,elm_ID,..)), provided these neighbouring elements are on
            the 'top' region of the surface element.
    NOTE: neighbouring solid elements in neigh do share nodes with the given surface
    element, but not an entire face (otherwise they would be attached)
    """
    ...
    df_elm_msk = df_elm[[bool(set.intersection(nall,set(L))) for L in df_elm['nodes'].values.tolist()]]
    ...
    for nel in df_elm_msk.index:
        vec = [i-j for i,j in zip(elem_ctrd(df_nod, df_elm.loc[nel], nodmap),elem_ctrd(df_nod, sr_elm, nodmap))]
        #dp = sum([i*j for i,j in zip(vec,ext_nrm(df_nod,sr_elm))])
        dp = sum([i*j for i,j in zip(vec, elm_nrm)])
        if dp > 0: neigh.append(nel)
...
def surf2vol(df_elm, elm_ID):
    ...
    for i in df_elm[(df_elm['type']==16) | (df_elm['type']==9)].index:
        ...
        if len(n) == s2inodes[df_elm.loc[i,'type']]: s_val.append(i)
...
def surf2surf(df_elm, elm_ID, nrm_dict):
    ...
    for k in nrm_dict.keys():
        if bool(set.intersection(set(df_elm.loc[k,'nodes']),set(df_elm.loc[elm_ID,'nodes']))):
            s_adj = k
            break
...
def part2surf(df_elm, srf_ID_top, srf_ID_bot, s_topbot):
    ...
    s_topbot   = List with the [top,bottom] SOLID element IDs attached to the active
                 crack in consideration (srf_ID_bot, as specified at upper level)
                 
    NOTE: It is assumed that surface elements becoming interfaces are never partitioned
          in gmsh alongside the solid elements, as gmsh only seems to deal with the
          highest dimension when partitioning. Should this behaviour change, this
          function would have to be revised!
    """
...
def make_crack(df_nod, df_elm, active_crack_ID, dic_s2s, dic_srf2srf, all_cracks_ID):
    ...
    if type(active_crack_ID) == int:
        ...
        df_crack_el = df_elm[[bool(set.intersection(set(active_crack_ID),set(L[:1]))) for L in df_elm.tags]]
    ...
    for i in df_crack_el.itertuples():
        ...
        if s_prev != 0:
            sgn = np.sign(sum([v1*v2 for v1,v2 in zip(nrm_dict[i[0]],nrm_dict[s_prev])]))
            ...
            if (sgn == 0):
                print('Surface {} is perpendicular to previous surfaces'.format(i[0]+1))
                print('Unable to orientate surface. Consider changing the surface ordering')
                sys.exit(1)
        ...
        s_topbot = srf_split(df_nod, df_elm, i[0], nrm_dict[i[0]], dic_s2s, node_mapping)
        ...
        if s_topbot[0] != 0:
            for j in range(len(df_elm.loc[s_topbot[0],'nodes'])):
                if df_elm.loc[s_topbot[0],'nodes'][j] in set(i[1]):
                    df_elm.loc[s_topbot[0],'nodes'][j] = node_mapping[df_elm.loc[s_topbot[0],'nodes'][j]]
            # Duplicate nodes in contact with active crack i belonging to surfaces
            # attached to the solid element s_topbot[0] that are NOT the active crack
            for ii in el_s:
                    if not op(active_crack_ID, df_elm.loc[ii,'tags'][0]):
                        for j in range(len(df_elm.loc[ii,'nodes'])):
                            if df_elm.loc[ii,'nodes'][j] in set(i[1]):
                                df_elm.loc[ii,'nodes'][j] = node_mapping[df_elm.loc[ii,'nodes'][j]]
                    # Update surface partition tags if necessary
                    if len(df_elm.loc[s_topbot[0],'tags'])>2 and df_elm.loc[ii,'tags'][0] == all_cracks_ID:
                        dnid = set([node_mapping[dn] for dn in df_elm.loc[i[0],'nodes']])
                        if dnid == set(df_elm.loc[ii,'nodes']):
                            part2surf(df_elm, ii, i[0], s_topbot)
                        
            # Duplicate nodes of neighbouring solid elements in contact with
            # active crack i but NOT attached to the crack plane (i.e. NOT attached to
            # any active crack)
            s_neigh = srf_neigh(df_nod, df_elm, i[0], nrm_dict[i[0]], dic_s2s, node_mapping)
            if not s_neigh: continue
            # Discard solid elements attached to other active cracks (outside of active crack i)
            # before proceeding to duplicate nodes
            for sn in s_neigh:
                proceed = True
                ...
                if proceed:
                    for j in range(len(df_elm.loc[sn,'nodes'])):
                        if df_elm.loc[sn,'nodes'][j] in set(i[1]):
                            df_elm.loc[sn,'nodes'][j] = node_mapping[df_elm.loc[sn,'nodes'][j]]
                    # Duplicate nodes on surfaces attached to solid element sn as well,
                    # if these share nodes with active crack i:
                    for s in s_sn:
                        for j in range(len(df_elm.loc[s,'nodes'])):
                            if df_elm.loc[s,'nodes'][j] in set(i[1]):
                                df_elm.loc[s,'nodes'][j] = node_mapping[df_elm.loc[s,'nodes'][j]]
        else:
    ...
    newID = len(df_elm)
    for i in df_crack_el.itertuples():
        n = df_elm.loc[i[0],'nodes'] # NOT i[0] as node order may have changed to maintain a consistent top/bottom
        newID += 1
...
def df2mesh(fname, df_phe, df_nod, df_elm):
    ...
    for e in df_elm.itertuples():
        line = str(e[0]) + ' ' + str(e[3]) + ' ' + str(len(e[2])) + ' ' + ' '.join([str(i) for i in e[2]]) + ' ' + ' '.join([str(i) for i in e[1]]) + '\n'
        f.write(line)
    f.write('$EndElements')
...
def main():
    ...
    # Argument sys[2] = crack physical tags, with some pairs being possibly coupled
    # if they belong to the same crack plane (e.g. [1,[2,3],4])
    crack_ids = []
    in_list = []
    ...
    # Argument sys[3] = physical tags of the solids in the mesh, paired as itertuples
    solid_physIDs = sys.argv[3].split(',')
    solid_physIDs = list(map(int, solid_physIDs))
    it = iter(solid_physIDs)
    solid_physIDs = dict(zip(it,it))
    ...
    if(not file_name):
        ...
        sys.exit(1)
    if(solid_physIDs == -1):
        print('Error: Must specify solid physical IDs')
        sys.exit(1)
